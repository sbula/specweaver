...
down_revision: str | Sequence[str] | None = "037b85034bb0"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "created_at", specweaver.core.config.database.StrictISODateTime(), nullable=False
        ),
        sa.Column(
            "updated_at", specweaver.core.config.database.StrictISODateTime(), nullable=False
        ),
        sa.ForeignKeyConstraint(
            ["project_name"], ["projects.name"], onupdate="CASCADE", ondelete="CASCADE"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "last_heartbeat_at", specweaver.core.config.database.StrictISODateTime(), nullable=True
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "resolved_at", specweaver.core.config.database.StrictISODateTime(), nullable=True
        ),
    ...
    with op.batch_alter_table("memory_state_transitions", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_memory_state_transitions_task_id"), ["task_id"], unique=False
        )

    op.create_table(
        "memory_task_dependencies",
