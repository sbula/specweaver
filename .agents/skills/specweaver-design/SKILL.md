...
> [!IMPORTANT]
> **Write the lossy half only.** The state — suite result, gate status, commits unpushed, open rows
> — is generated by `python scripts/session_handover.py`, which rewrites the block between its
> markers and returns every other line byte for byte. Numbers copied by hand are the ones that go
> stale, so copy none of them.
>
> **A decision that outlives the session belongs in `.agents/STATE.md`, not the handover.**
> `.tmp/HANDOVER.md` is gitignored and session-scoped; `STATE.md` is committed and says where the
> project stands. A capability left unapproved, or a correction owed, goes in `STATE.md`.
