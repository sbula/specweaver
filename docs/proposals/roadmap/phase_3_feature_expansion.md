...
| **3.22** ✅ | Polyglot AST Skeleton Extractor | _(new)_ | _Pivoted from Context Ledger (304 Caching) to prevent LLM memory hallucination._ Provides `read_skeleton`, `read_symbol`, and AST mutation capabilities via Tree-Sitter. **Complete**: SF-1 (Read) across 5 languages. SF-2 (Write) Python completed. |
