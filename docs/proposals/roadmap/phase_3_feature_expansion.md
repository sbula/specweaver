...
| **3.12b** ✅ | Static model routing (config-driven) | _(split from original 3.12)_ | Map task types to models in config: `review → claude`, `implement → gemini-pro`. Uses 3.12a's registry to resolve provider+model per task_type. No AI, no learning — pure user configuration. **Complete:** 3548 tests total. See [LLM routing & cost analysis](../../analysis/llm_routing_and_cost_analysis.md). |
