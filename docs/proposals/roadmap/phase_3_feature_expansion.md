...
| **3.12** 🔧 | Token & cost telemetry | _(split from original 3.12)_ | Log `model_id`, `prompt_tokens`, `completion_tokens`, `estimated_cost` on every LLM call. **Core engine complete**: `TelemetryCollector` (decorator), `UsageRecord`/`estimate_cost()`, `TelemetryMixin` (DB persistence), schema v9, factory telemetry wrapping, task_type attribution in flow handlers. 3423 tests. **Remaining**: `sw usage` CLI, `sw costs` CLI, runner flush integration. See [implementation plan](phase_3/feature_3_12_implementation_plan.md). |
| **3.12a** | Multi-provider adapter registry | _(split from original 3.12)_ | Extend `LLMAdapter` ABC with adapters for Claude, Qwen, Mistral, OpenAI. Registry maps provider names to adapter classes. Config selects provider. See [LLM routing & cost analysis](../../analysis/llm_routing_and_cost_analysis.md). |
| **3.12b** | Static model routing (config-driven) | _(split from original 3.12)_ | Map task types to models in config: `review → claude`, `implement → gemini-pro`. No AI, no learning — pure user configuration. See [LLM routing & cost analysis](../../analysis/llm_routing_and_cost_analysis.md). |
...
| **3.14** | Spec-to-code traceability | `future_capabilities_reference.md` §17 | Bidirectional linking. _Note: Will merge with Artifact Lineage Graph from Phase 4 — see [LLM routing & cost analysis](../../analysis/llm_routing_and_cost_analysis.md)._ |
