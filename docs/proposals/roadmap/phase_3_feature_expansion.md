...
| **3.22** ✅ | Polyglot AST Skeleton Extractor | _(new)_ | _Pivoted from Context Ledger (304 Caching) to prevent LLM memory hallucination._ Provides `read_skeleton` and `read_symbol` tools via Tree-Sitter to drastically reduce token payload and "Context Window Bloat". **Complete**: SF-1 (Commons Layer) implemented natively across 5 languages. 3769 total tests. |
