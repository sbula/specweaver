...
| **3.12** | Multi-model LLM support (dynamic routing) | `future_capabilities_reference.md` §9, §15 | Route prompts to best model per task by result/cost ratio (Gemini, Claude, Qwen, Mistral, OpenAI) We must define an effort/result metrik (cheap model resulting in complete, unwanted refactoring 20 steps later is not as good as an expensive one withoet refactoring); interface already abstracted |
