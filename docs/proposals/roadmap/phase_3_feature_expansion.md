...
| **3.11b** | Post-3.5 validation override consolidation | _(deferred from 3.5)_ | Consolidate validation override mechanisms. DB `validation_overrides` table (3.3) and validation sub-pipeline YAML inheritance (3.4) both configure per-project rule thresholds. Decide: sub-pipelines as single source of truth, or DB overrides as runtime layer on top? |
