...
| **3.24** | Reverse-Weaving (`sw capture`) | _(new)_ | Archaeology tool for brownfield adoption. Uses AST "Skeleton Extraction" (signatures + Javadocs) to draft baseline `Spec.md` contracts from legacy DB/Java code. all supported languages |
| **3.25** | Automated iterative decomposition (multi-level) | `future_capabilities_reference.md` §18 | Builds on basic foundation: DMZ-style iterative loop, automated quality gates, recursive decomposition (feature → sub-features → components). |
| **3.26** | Router-based flow control | _(new)_ | Conditional branching in pipelines — route specs to different pipeline paths based on assessment (e.g., simple → fast track, complex → full decomposition). New `router` YAML key on steps. |
| **3.27** | Multi-spec pipeline fan-out | _(from 3.1 analysis)_ | Sub-pipeline spawning: decomposition outputs N component specs, each runs its own L3 pipeline. Parent pipeline waits for all children. |
...
| **3.28j** | ↳ Feedback loop & retry | _(subfeature)_ | Coding agent gets stack traces + spec references. Scenario agent gets expected vs actual + spec references. Neither sees other's code. Loop back if fixable, HITL escalation if spec ambiguity. |
| **3.29** | Archetype-Based Rule Sets | _(new)_ | Auto-provisioned rules for specific architectural profiles (`kotlin-service`, `rust-worker`) to enforce framework-specific standards inherently. |
| **3.30** | Macro & Annotation Evaluator | _(new)_ | Specialized indexer capable of unrolling Rust Procedural Macros (`#[derive]`) and Kotlin Compiler Plugins (Spring Boot annotations) so the LLM understands the true runtime reality, not just the raw signature. |
| **3.31** | Protocol & Schema Analyzers | _(new)_ | Native parsing of `.proto` (gRPC), `openapi.yaml`, and AsyncAPI files to catch contract drift across polyglot microservices. |
...
| **3.43** | Git Worktree Bouncer (Sandbox) | _(new)_ | Provides dictatorial validation while fully supporting native IDEs. Clones current target to a `git worktree` for the Agentic IDE. Mathematical diff striping auto-rejects and deletes LLM hallucinations to forbidden files before merge. |
