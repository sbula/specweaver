...
| **3.19** 🏃 | Polyglot TestRunner Interface | _(new)_ | Wraps target-language CLI commands (`cargo`, `gradlew`, `pytest`) into a unified `LanguageRunnerInterface`. Treats execution as a Black Box (validating exit codes/stderr) to prevent Python AST hardcoding. |
| **3.20** | Dynamic Risk-Based Rulesets | _(new)_ | Injects strict constraints (deterministic math, 95% coverage) or relaxed constraints (standard DB types, 70% coverage) into the fixed 10-test battery based on the target module's domain risk (DAL A vs DAL E). |
