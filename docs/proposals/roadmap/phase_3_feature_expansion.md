...
| **3.12a** ✅ | Multi-provider adapter registry | _(split from original 3.12)_ | Auto-discovery registry: each adapter is self-describing (`provider_name`, `api_key_env_var`, `default_costs`). System scans `llm/adapters/` at import → builds registry automatically. Adding a new provider = one file, zero other changes. **(Commit 1: Registry & Schema V10, Commit 2: 2026 Model Standards & Edge Cases Complete)** |
