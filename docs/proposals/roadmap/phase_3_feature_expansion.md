...
| **3.12** | Token & cost telemetry | _(split from original 3.12)_ | Log `model_id`, `prompt_tokens`, `completion_tokens`, `estimated_cost` on every LLM call. DB table + `sw usage`. Zero deps, instant visibility. See [LLM routing & cost analysis](../../analysis/llm_routing_and_cost_analysis.md). See [implementation plan](phase_3/feature_3_12_implementation_plan.md). |
