...
#### [NEW] `src/specweaver/loom/commons/test_runner/kotlin.py`
- Inherits `TestRunnerInterface`.
...
#### [NEW] `tests/unit/loom/commons/test_runner/test_java.py`
...
#### [NEW] `tests/unit/loom/commons/test_runner/test_kotlin.py`
- Implements isolated validation logic ensuring `TestRunnerInterface` conformance exactly equivalent to java.py while validating its Kotlin `detekt` payload maps appropriately to SARIF blocks cleanly.

## Verification Plan

### Automated Tests
- Run `pytest tests/unit/loom/commons/test_runner/test_java.py tests/unit/loom/commons/test_runner/test_kotlin.py` natively to validate parser extraction correctly isolates `LintError` paths from standard payloads cleanly without a valid CLI trigger cleanly.
