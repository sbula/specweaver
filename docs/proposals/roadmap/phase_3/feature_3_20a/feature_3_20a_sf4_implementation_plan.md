...
- Rewrite `from specweaver.llm.adapters import ...` to `from specweaver.llm.adapters.registry import ...` inside `src/specweaver/llm/factory.py`, `src/specweaver/llm/telemetry.py`, `src/specweaver/llm/router.py`.
...
Consumers previously calling `from specweaver.loom.tools.git import GitTool` must explicitly call `from specweaver.loom.tools.git.tool import GitTool`. All boundary verification relies strictly on Tach.
