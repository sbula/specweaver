...
- **Implementation Plan**: docs/proposals/roadmap/phase_3/feature_3_20a/feature_3_20a_sf5_implementation_plan.md
- **Status**: APPROVED
...
**Action:** Remove the manual testing. Ensure `src.specweaver.validation` is registered in `tach.toml` `interfaces`, expressly omitting `runner` from exports to enforce CI-level bounding instead of file-level crashes.

### 2.2 Boilerplate Verification Subsumption
`tests/unit/test_architecture.py` contains script logic asserting that `src.specweaver.api` is not mapped incorrectly.
