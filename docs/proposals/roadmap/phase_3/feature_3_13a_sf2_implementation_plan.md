...
- **Implementation Plan**: docs/proposals/roadmap/phase_3/feature_3_13a_sf2_implementation_plan.md
- **Status**: DRAFT
...
- **Action**: Add a `@classmethod` factory `create_single_step(name, action, target, gate)` to `PipelineDefinition`.
- **Purpose**: Creates an in-memory 1-step pipeline structurally identical to parsed YAML pipelines. It will be the single mandated entry point anywhere single-shot pipelines are required by the system.
