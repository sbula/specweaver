...
| **4.5a** | Task-type cost analytics dashboard | _(split from original 3.12)_ — Aggregate telemetry from 3.12 into cost breakdown by task type (draft/review/plan/implement) across models. Data-driven model selection insights. See [multi-model analysis](phase_3/multi_model_analysis.md). |
| **4.5b** | Artifact lineage graph | _(split from original 3.12, merges with 3.14)_ — Tag every artifact with `artifact_uuid`, `parent_uuid`, `model_id`. Build directed lineage graph: spec → plan → code → tests. Foundation for friction detection and provenance. See [multi-model analysis](phase_3/multi_model_analysis.md). |
| **4.5c** | Deterministic friction detection | _(split from original 3.12)_ — When downstream agent modifies >20% of upstream scaffolding (measured by `git diff`), flag "friction event" and attribute to upstream model. No LLM needed — pure diff math. See [multi-model analysis](phase_3/multi_model_analysis.md). |
| **4.5d** | Data-driven routing recommendations | _(split from original 3.12)_ — Analyze telemetry + friction data to **suggest** (not auto-apply) model swaps. "Model X has 3× more friction on planning tasks than Model Y." See [multi-model analysis](phase_3/multi_model_analysis.md). |
