...
* **Security Defenses:** 🔧 **Built, not approved.** Integrates `E-VAL-03` 🔧 (AST Prompt Injection Sanitization).

  > The group never had a design of its own, and does not need one: `E-VAL-03` owns the scope as
  > `FR-1`..`FR-4`, including the seam at `FilePromptAdapter` where analysed source enters a prompt.
  > Proof lives with those FRs, not in a separate entry restating them.
