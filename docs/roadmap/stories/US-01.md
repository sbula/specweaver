...
* **Security Defenses:** ⚰️ **Retired 2026-08-21.**

  > `E-VAL-03` was ruled nonsense under the [benefit review](../../analysis/benefit_chain_analysis_2026-08-20.md) §8 and retired by the user: the filter
  > blocked attacks the sandbox already survives, missed semantic poisoning, and — best-effort — nothing could rely on it. Its detector and tests are
  > deleted; the structural half (`escaping.py`) remains delivered under `E-INTL-01`. Terminal: no successor owns *this* scope. The slot's benefit-positive
  > re-reading (structure INTO prompts) lives on as `B-SENS-09`.
...
* **Speed & Security (Rust):** ⚰️ **Descoped 2026-08-20.**
  ...
  > The 2026-08-13 `ADR-003` step had moved this scope to `A-VAL-04`. The 2026-08-20
  > [benefit review](../../analysis/benefit_chain_analysis_2026-08-20.md) then judged that capability a dead end — validation is milliseconds beside the LLM
  > calls dominating every pipeline — and descoped the work with it. Terminal: no successor owns this scope, deliberately.

* **Rubrics-as-Content:** 🔧 **Built, not approved.** Integrates `C-VAL-05` 🔧 — the DAL-gated rubric files
  are wired into the live review surface.

  > `C-VAL-05` owns the scope as `FR-1`..`FR-5`, and proves the wiring end to end in
  > `tests/e2e/capabilities/assurance/test_project_rubric_reaches_the_reviewer_e2e.py`.
  > The battery half of the original scope had nothing to wire: all 23 rules are mechanical, and
  > `S03`/`S07` are `requires_llm = False`. The design records that measurement.
