...
* **Verifiable Proof:** P-3 and P-4 by
  ...
  proven by the first link of that same chain — emptying the `context.yaml` walk fails it, so the
  declared boundaries really are what the graph is built from; P-2 is proven by
