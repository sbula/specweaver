...
| P-3 | Journey: a query returns the right context from a federated graph without blowing up the context window | cross-feature | `A-SENS-02` | retired | `ADR-003` — owned by `A-SENS-02`, which declares this seam as its own FR. `B-SENS-03` 🔧 delivered the chunking half; the journey still needs the storage and retrieval half |
...
* **Infinite Scale Management:** the add-on's paths under `ADR-005` — its path
inventory and any
