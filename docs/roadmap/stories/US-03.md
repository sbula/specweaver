# US-03: Autonomous Implementation
...
* **Verifiable Proof (delivered by SF-03, `64d44a71`):**
  `tests/e2e/capabilities/sandbox/test_implement_loop_worktree_isolation_e2e.py` +
...
* **Multi-Language Test Support:** *Pending Design.* Integrates `D-VAL-03` (Polyglot QA Runner, built ✅) into the `sw implement` loop for non-Python targets.
...
* **Visual UI Drift Detection:** *Pending Design.* Blocked on `A-VAL-05` (Multi-Modal Visual Quality Gates, unbuilt).
...
* **Graduated Autonomy (the "middle way" dial):** 🔧 **Built, not approved.** Integrates `C-FLOW-11` 🔧 — the
  `mode: oneshot | agentic` dial, its DAL ceiling, and the bounded work unit behind a replaceable
  `AgentRuntime`. **Add-on ID — distinct from the base contract's internal "INT-US-03 SF-03"
  sub-feature, which is committed** (SF vs sub-story numbering live in different namespaces; stated
  per the SF/CB terminology rule).

  > **The `sw implement` inner loop is NOT converted, and that is deliberate** — it is a Non-Goal in
  > `C-FLOW-11`'s design, not an oversight. The dial exists and every shipped pipeline resolves to
  > `oneshot`; turning it on for a real pipeline changes how that pipeline produces work, and is a
  > decision to take on its own evidence rather than as a side effect of the mechanism landing.
  > Whoever takes it owns the pilot's own proof.
