...
* **Dynamic Risk Controls**
  * **Status:** ✅ Complete (2026-08-19) — P-9 closed by `TECH-041`
  * **Verifiable Proof:** `tests/e2e/capabilities/assurance/test_code_dal_strictness_e2e.py` runs one
    module twice under `DAL_E` and `DAL_A` with everything else held equal: the lenient run exits 0
    reporting warnings, the strict run exits 1 on the identical warnings. Three mutants die,
    including the always-strict inverse the lenient run exists to catch.
    `tests/integration/core/flow/handlers/test_code_validation_dal_strictness.py` shows the same on
    the pipeline path.

    **P-9 was worded as *which rules run* and is closed as *the verdict*, which is the honest
    difference.** A DAL does not select a rule set; it decides how the same findings are judged. Rule
    selection by DAL is not implemented and is not claimed by any of the three capabilities' FRs.
  ...
  * **Verifiable Proof:** [Pending]
  ...
  | P-9 | Journey: a DAL declared in `context.yaml` changes the verdict on a specific file, end to end | cross-feature | `C-VAL-03` | yes — **done** | — |
