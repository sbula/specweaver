...
* **Verifiable Proof:** P-1 and P-2 are cited only at unit tier and owe a spanning test —
  `check_fr_coverage.py C-FLOW-03` and `... B-SENS-02` both exit 0, which attributes the claims
  without crossing the seam. The add-on's P-1 is proven by
