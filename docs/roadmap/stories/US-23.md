...
* **Verifiable Proof:** P-1's boot, fetch and inject by
  ...
  `environment_context` the generator receives. Parsing `mcp_servers` out of a `context.yaml` is
  cited at unit tier only. Also `check_fr_coverage.py C-INTL-02`, which exits 0 with all four FRs
behind
  killed mutants.
