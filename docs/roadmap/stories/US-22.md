...
* **Verifiable Proof:** P-2 by
  ...
  `@trace` is added, with a mutant behind each half. P-1 is cited at unit tier only and owes a
  spanning test: `A-VAL-01`'s parsers are proven in isolation, and nothing drives a `.proto` on disk
  through to a contract-drift finding. Both also by `check_fr_coverage.py A-VAL-01` and
