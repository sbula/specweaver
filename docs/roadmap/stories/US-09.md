...
* **Containerized Isolation**
  * **Status:** ⬜ Open — `B-EXEC-01`'s nine FRs and `D-EXEC-01`'s are uncited against the container
    path, and `D-EXEC-01` has no design document at all
  ...
  * **Verifiable Proof:** `tests/integration/sandbox/execution/test_container_executor_integration.py`
    drives the real executor against live podman across six project shapes; what is missing is the
    citation of those FRs, not the exercise.
