...
* **`C-SENS-06` 🔮: Event-Sourced 4D Graph**<br>
  > _(new)_ | Upgrades the SQLite Knowledge Graph to be event-sourced (valid_from/valid_to tracking). Allows semantic `git bisect` and point-in-time architectural queries without executing git
  > checkouts.
