...
* **`E-VAL-04` 🔜: Multi-Stage Reviews**
  ...
  > - **Needs:** `C-VAL-05` → **approved**, as the rubric substrate · `E-VAL-02` · `B-VAL-02`
...
* **`D-VAL-04` ✅: Adaptive Assurance Standards** (Legacy: 3.32a)
  ...
  > - **Needs:** `E-VAL-02` → the analyzer it toggles
...
* **`C-VAL-03` ✅: Dynamic Risk Rulesets** (Legacy: 3.20b)
  ...
  > - **Needs:** `D-VAL-02` → pipeline inheritance · the module's DAL
...
* **`C-VAL-04` ✅: Traceability Matrix Check** (Legacy: 3.21)
  ...
  > - **Needs:** `D-SENS-02` → the test file AST
  ...
  > - **Cannot do:** vouch for test *quality*. The tag is written by the same model as the test, so a hallucinated test carries a well-formed tag. Quality is `A-VAL-03`'s job

* **`C-VAL-05` 🔧: Rubrics-as-Content Validation**
  > - **Purpose:** Change what a review asks for by editing a file, not by changing code and cutting a release
  > - **Trigger:** When a review step loads its criteria
  > - **Needs:** `E-INTL-03` → the review engine · the run's DAL, to pick a variant
  ...
  > - **Enables:** `E-VAL-04` · `B-VAL-03` · `B-INTL-08` · `D-INTL-04` · `D-INTL-07` — the substrate they build on
  > - **Done when:** editing a rubric changes the next review's verdict, with no code change
  > - **Thin content:** three rubric files ship and no project override exists anywhere. `FR-2`–`FR-4` serve a user who does not exist yet
...
* **`C-VAL-06` 🔜: Structural Code-Health Rules**
  ...
  > - **Needs:** `D-SENS-02` → parsed structure
  > - **Reads:** source files
  ...
  > - **Done when:** 🔴
  > - **Out of scope:** LCOM4 and coupling (`B-VAL-06`) · mutation (`A-VAL-03`) · the DAL policy layer (`C-VAL-03`)
...
* **`B-VAL-02` ✅: Spec Rot Interceptor** (Legacy: 3.23)
  ...
  > - **Needs:** `B-VAL-01` → drift detection
...
* **`B-VAL-03` 🔜: Semantic Completeness Review** (Legacy: 3.42)
  ...
  > - **Needs:** `C-VAL-05` → the rubric substrate · `E-INTL-01` → the model
  > - **Reads:** the generated test suite and the spec
  > - **Produces:** 🟡 ERRORs for missing branch coverage, as rule `C10`
  > - **Enables:** 🔴
  > - **Done when:** 🔴
  > - **Design direction:** rubric-first. The `C10` rule class is a thin engine shim; the completeness criteria live in a versioned, DAL-gated rubric file, not in Python
...
* **`B-VAL-04` 🔜: SWE-Bench QA Gates** (Legacy: 3.47)
  ...
  > - **Needs:** `D-FLOW-01` → the pipelines under test
  ...
  > - **Done when:** 🔴
  > - **Hard requirement:** model-pinned, multi-run, with known variance. **Unpinned, the score measures the plugged-in model rather than the platform**
...
* **`B-VAL-05` 🔜: DAL Architecture Gate**
  ...
  > - **Needs:** `B-SENS-02` → the persistent knowledge graph · DAL assignments
  > - **Reads:** a package's dependencies
  > - **Produces:** 🟡 a rule finding on a boundary violation
  > - **Enables:** 🔴
  > - **Done when:** 🔴
  > - **Gate:** do not design per-level behaviour yet. Every live DAL consumer decides strict-or-relaxed only; the tier count is an empirical question the trading system answers
...
* **`B-VAL-06` 🔜: Cohesion & Coupling Metrics (LCOM4, CBO, Instability)**
  ...
  > - **Needs:** `C-VAL-06` → the cheap rules it must not gate · 🟡 tooling that does not exist for Python
  > - **Reads:** source structure
  > - **Produces:** 🟡 LCOM4 components · CBO · instability
  > - **Enables:** refactoring guidance rather than a warning
  > - **Done when:** 🔴
  > - **Open question:** gate, or advice. Undecided
...
* **`B-VAL-07` 🔜: Graph-Invariant Verification**
  ...
  > - **Needs:** `B-SENS-02` → the graph · `TECH-068` → real edges
  ...
  > - **Done when:** 🔴
  > - **Honest limit:** graph-**checked**, not guaranteed, on dynamic languages. Reads traversal only, never vector output
...
* **`A-VAL-01` ✅: Protocol/Schema Analyzers** (Legacy: 3.31)
  ...
  > - **Needs:** `D-SENS-02` → the parser layer
...
* **`A-VAL-02` 🔜: Symbolic Math Validation** (Legacy: 3.39)
  ...
  > - **Needs:** 🟡 the spec's stated formulas
  > - **Reads:** generated code and spec formulas
  > - **Produces:** 🟡 transcription findings
  > - **Enables:** the trading system journeys (`US-13`, `US-18`)
  > - **Done when:** 🔴
  > - **Re-scoped 2026-08-20:** the ML half — "verify FinBERT calculations" — was removed. A neural net's outputs cannot be formally verified, and "proves secure / discovers 0-days" promised
...
* **`A-VAL-03` 🔜: Mutation Testing Gates** (Legacy: 4.7)
  ...
  > - **Needs:** `D-VAL-01` → the test runner
...
* **`A-VAL-05` 🔜: Multi-Modal Visual Quality Gates (V-Series)** (Legacy: 4.11)
  ...
  > - **Needs:** 🟡 a vision model · headless browser rendering
...
* **`A-VAL-06` 🔜: Industry Standard Bridges** (Legacy: 3.41)
  ...
  > - **Needs:** 🟡 `A-VAL-01` → the contract parsers
