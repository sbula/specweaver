...
* **`E-INTL-03` ✅: Spec Review Engine** (Legacy: Step 4)
  ...
  > - **Needs:** `E-INTL-01` → the model · `C-VAL-05` → the criteria it judges against
...
* **`D-INTL-01` ✅: Implementation Generator** (Legacy: Step 5)
  ...
  > - **Needs:** `E-INTL-01` → the model · the validation battery
  > - **Reads:** the spec
...
* **`D-INTL-04` 🔜: Design Questionnaire** (Legacy: 3.52)
  ...
  > - **Needs:** `C-VAL-05` → rubric content, as the question source
  ...
  > - **Done when:** 🔴
  > - **Design direction:** a rhythm harness plus rubric content — the grill pattern, one question at a time, facts self-served, decisions to the user. **Not hardcoded question trees**
...
* **`D-INTL-05` ✅: Project Metadata Injection** (Legacy: 3.15)
  ...
  > - **Needs:** `E-FLOW-01` → project config
...
* **`D-INTL-06` ✅: Context Hydration & Handover Engine**
  ...
  > - **Needs:** `B-INTL-09` → the Memory Bank
...
* **`D-INTL-07` 🔴: Agentic Interview Drafting (Grill-Style)**
  ...
  > - **Needs:** `C-FLOW-11` → **BLOCKED on it** · `C-VAL-05` → soft dependency, rubric content
...
* **`D-INTL-08` 🔜: Polyglot Implementation Loop**
  ...
  > - **Needs:** `D-VAL-03` → the five shipped language runners
  ...
  > - **Measured:** paths, artifact tags and the fence stripper are hardcoded to `.py` at four points, and no `--language` flag exists. Builds no runner — routes to the ones that exist
...
* **`C-INTL-01` ✅: Iterative Decomposition** (Legacy: 3.24)
  ...
  > - **Needs:** `D-INTL-02` → the basic decomposition
  > - **Reads:** the feature spec and each pass's output
  > - **Produces:** feature → sub-features → components
  > - **Enables:** `C-FLOW-12` execution
  > - **Done when:** 🔴
  > - **Known wrong:** designed as multi-level and **shipped single-pass**. `AD-2` and the agent-sized split heuristic were never built and never descoped — that is `C-INTL-07`
...
* **`C-INTL-06` 🔜: Envelope-vs-Content Prompt Externalization**
  ...
  > - **Needs:** `C-INTL-05` → profiles · `C-FLOW-11` → work units that read the files directly
...
* **`C-INTL-07` 🔜: Multi-Level Recursive Decomposition**
  ...
  > - **Needs:** `D-INTL-02` · `C-INTL-01` → what shipped
...
* **`B-INTL-02` ✅: Macro Evaluator** (Legacy: 3.30)
  ...
  > - **Needs:** `D-SENS-02` → the parsed AST
...
* **`B-INTL-03` 🔜: Synthetic Commons** (Legacy: 3.51)
  ...
  > - **Needs:** `D-INTL-02` → the decomposition
...
* **`B-INTL-04` 🔮: Dynamic AI Arbiter** (Legacy: 5.8)
  ...
  > - **Trigger:** 🔴
  > - **Needs:** a persistent knowledge graph · labelled training data (`C-FLOW-07`) · a solution to credit assignment
...
* **`B-INTL-05` 🔜: Dynamic Tool Gating via Archetypes** (Legacy: 3.30a)
  ...
  > - **Needs:** `B-INTL-01` → archetypes · `C-FLOW-11` → the role model
  ...
  > - **Done when:** 🔴
  > - **Design direction:** the tool-allowlist half of "role = tools + mounted skills + DAL-scoped gates". Skill-mounting is the other half — design jointly with `C-FLOW-11`
...
* **`B-INTL-06` 🔜: Multi-Agent Isolation Patterns** (Legacy: 4.5)
  ...
  > - **Needs:** `C-FLOW-11` → single-agent work units · `C-EXEC-06` → session isolation
  > - **Reads:** —
...
* **`B-INTL-07` 🔜: Error Attribution Arbiter**
  ...
  > - **Needs:** `B-FLOW-01` → the JOIN gate · `E-INTL-01` → the model
...
* **`B-INTL-08` 🔮: Semantic Code Review**
  ...
  > - **Needs:** `B-VAL-07` → the graph diff · `C-VAL-05` → review criteria as rubric content
  > - **Reads:** the graph diff and the change
  > - **Produces:** 🟡 a review narrative accompanying the diff
  > - **Enables:** 🔴
  > - **Done when:** 🔴
  > - **Re-worded 2026-08-21:** "replaces text diffs" named a consumer that cannot exist — nobody reads a dataflow-graph diff instead of code
...
* **`B-INTL-10` 🔮: Declarative Prompt Optimization**
  ...
  > - **Needs:** `C-FLOW-01` → telemetry · `E-FLOW-01` → profile storage
  > - **Reads:** 🟡 telemetry, routing and active models
  > - **Produces:** 🟡 compiled prompt profiles
  > - **Enables:** 🔴
  > - **Done when:** 🔴
  > - **May be superseded:** premised on owning slot-prompt assembly — the layer `C-INTL-06` and `C-FLOW-11` shrink. At design time, re-scope onto rubric content or retire
...
* **`A-INTL-01` 🔜: Adversarial Spec Review** (Legacy: 3.50)
  ...
  > - **Needs:** `E-INTL-01` → the model · 🟡 the arbiter agent
  > - **Reads:** the spec
...
* **`A-INTL-02` 🔜: LLM Symbolic Execution** (Legacy: 4.14)
  ...
  > - **Trigger:** 🔴
  > - **Needs:** `D-INTL-08` → the polyglot implement loop · a real C/C++ or Rust target
  > - **Reads:** native code
  > - **Produces:** 🟡 memory-safety defect reports, via KLEE
  > - **Enables:** 🔴
  > - **Done when:** 🔴
  > - **Gate:** KLEE targets native code. The trading system as planned is not such a target
...
* **`A-INTL-04` 🔜: Memory Consolidation** (Legacy: 5.7)
  ...
  > - **Needs:** `B-INTL-09` → the memory bank · `E-INTL-01` → the model that decides
...
* **`A-INTL-05` 🔜: Multi-Repo Refactoring Orchestration**
  ...
  > - **Trigger:** 🔴
  > - **Needs:** 🟡 `A-SENS-04` → the federated system graph
