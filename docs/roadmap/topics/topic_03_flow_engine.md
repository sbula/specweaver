...
* **`D-FLOW-01` ✅: Pipeline Runner** (Legacy: Step 11)
  ...
  > - **Precondition:** `E-FLOW-02` → the parsed step model
...
* **`D-FLOW-02` ✅: sw run CLI** (Legacy: Step 13)
  ...
  > - **Precondition:** `D-FLOW-01` → the runner
...
* **`D-FLOW-03` ✅: Static Model Routing** (Legacy: 3.14)
  ...
  > - **Precondition:** `E-FLOW-03` → the provider registry
...
* **`C-FLOW-01` ✅: Cost Telemetry** (Legacy: 3.12)
  ...
  > - **Precondition:** `E-FLOW-03` → provider and cost data
...
* **`C-FLOW-03` ✅: Multi-Spec Fan-Out** (Legacy: 3.27)
  ...
  > - **Precondition:** `D-SENS-01` → the topology graph, to predict blast radius
...
* **`C-FLOW-04` 🔜: Work Packet Bundling** (Legacy: 3.49)
  ...
  > - **Precondition:** `C-FLOW-03` → the fan-out it optimises
...
* **`C-FLOW-05` ✅: Interactive Gate Variables (HITL)** (Legacy: 3.26c)
  ...
  > - **Precondition:** `D-FLOW-01` → gate state
...
* **`C-FLOW-06` ✅: Refactoring Phase 3 Optimizations** (Legacy: 3.32d)
  ...
  > - **Precondition:** `D-SENS-02` → AST skeletons · `D-SENS-01` → impact data
...
* **`C-FLOW-07` 🔜: HITL Root-Cause Tagging** (Legacy: 5.5a)
  ...
  > - **Precondition:** 🟡 `B-FLOW-03` → friction events to attach a reason to
...
* **`C-FLOW-08` 🔜: Pluggable Webhook & CI Invocation**
  ...
  > - **Precondition:** `D-FLOW-01` → run outcome
...
* **`C-FLOW-09` 🔜: DAL CI/CD Risk Evaluation**
  ...
  > - **Precondition:** `D-SENS-01` → the topology graph · DAL assignments
  > - **Reads:** the changed imports
  > - **Produces:** 🟡 an accept/reject verdict on the change
  > - **Enables:** 🔴
  > - **Done when:** 🔴
  > - **Limits:** per-level behaviour is not designed yet. Every live DAL consumer decides strict-or-relaxed only; the real tier count is an empirical question the trading project answers
...
* **`C-FLOW-10` 🔜: Deferred Router Mapping** (Legacy: 3.25)
  ...
  > - **Precondition:** `C-FLOW-02` → basic routing · `D-FLOW-01` → state persistence
...
* **`C-FLOW-11` 🔧: Graduated Autonomy — DAL-Driven Execution-Mode Dial**
  ...
  > - **Precondition:** `D-FLOW-01` → the runner · the run's DAL · `B-FLOW-05` → the spend ceiling
  ...
  > - **Limits:** `sw implement` still runs one-shot, so no user path reaches `agentic` mode
...
* **`C-FLOW-12` 🔜: Autonomous DAG Execution**
  ...
  > - **Precondition:** `C-EXEC-07` · `TECH-014` → **sequenced behind both**
...
* **`B-FLOW-02` 🔜: OpenTelemetry Agent Tracing** (Legacy: 3.44)
  ...
  > - **Precondition:** `D-FLOW-01` → the run hierarchy
...
* **`B-FLOW-03` 🔜: Friction Detection** (Legacy: 4.5c)
  ...
  > - **Precondition:** `C-FLOW-01` → telemetry to attribute against
...
* **`B-FLOW-04` 🔜: Hybrid RAG Orchestration** (Legacy: 5.4)
  ...
  > - **Precondition:** `A-SENS-02` → vectors · `B-SENS-03` → chunks
  ...
  > - **Done when:** 🔴
  > - **Note:** its output never feeds a gate or a blast radius. No correctness decision consumes vector output (`ADR-006` decision 3)
...
* **`B-FLOW-05` 🔧: Token-Burn Circuit Breakers**
  ...
  > - **Precondition:** `C-FLOW-01` → token and cost data · a decision on where LLM limits live — see its design and `STATE.md`
...
* **`A-FLOW-01` 🔜: Data-Driven Routing** (Legacy: 4.5d)
  ...
  > - **Precondition:** `C-FLOW-01` → telemetry · `B-FLOW-03` → friction events
...
* **`A-FLOW-02` 🔜: Hash GC** (Legacy: 5.3)
  ...
  > - **Trigger:** 🔴
  > - **Precondition:** `A-SENS-01` → hashes · `B-SENS-02` → the nodes
...
* **`A-FLOW-03` 🔜: Dead Code Detection & Analysis** (Legacy: 5.9)
  ...
  > - **Trigger:** 🟡 On demand
  > - **Precondition:** `B-SENS-02` → reachability edges · `A-SENS-02` → the Postgres graph at scale
...
* **`A-FLOW-04` 🔜: Blast-Radius Circuit Breaker**
  ...
  > - **Precondition:** `B-SENS-02` → graph traversal · `TECH-068` and `B-SENS-08` → complete edges
