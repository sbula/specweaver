...
* **Status:** ⬜ Pending — migration `INT-US-26-MIG` discharged 2026-08-17; no path
...
| P-3 | Journey: an OSV advisory resolves to every call site in the graph | cross-feature | `B-SENS-06` | retired | `ADR-003` — owned by `B-SENS-06`, which declares this seam as its own FR |
...
**`INT-US-26-MIG` is discharged, and every deferred path now names the ticket that owns it.**
