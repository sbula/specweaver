# US-26: Fleet-Wide CVE Remediation - Integration Contracts

## Base Story Contract (`INT-US-26`)
* **Status:** ⬜ Pending — migration `INT-US-26-MIG` discharged 2026-08-17; the contract stays open
* **Integration Description:** US-26's only closed capability is `B-SENS-02` (Persistent Knowledge
...
| P-3 | Journey: an OSV advisory resolves to every call site in the graph | cross-feature | this contract, deferred | no | `B-SENS-06` |
...
P-3 generates no FR yet: `B-SENS-06` (OSV Vulnerability Feed Ingestion) is unbuilt, so its interface
...
**`INT-US-26-MIG` is discharged; the contract stays open.** A migration finishes; a contract keeps
its
deferred rows until every path in the story is proven. US-26 closes when `B-SENS-06` lands and P-3
is
written.
