...
* **`E-EXEC-02` 🔜: Air-Gapped Network Egress Control**
  ...
  > - **Needs:** 🟡 `B-EXEC-01` → the container boundary to enforce it in
  > - **Reads:** —
...
* **`C-EXEC-02` ✅: Native CLI Nodes** (Legacy: 3.40)
  ...
  > - **Needs:** `E-SENS-03` → the `FolderGrant` that protects the script directory
...
* **`C-EXEC-04` 🔜: Concurrent Git Merge Orchestration**
  ...
  > - **Needs:** `C-FLOW-03` → the fan-out that produces them · `D-SENS-02` → the ASTs to merge
...
* **`C-EXEC-06` ✅: Per-Run (Session) Worktree Isolation**
  ...
  > - **Needs:** `D-EXEC-02` → the worktree machinery
  ...
  > - **Why DAL-C, not DAL-D:** it accumulates a whole run's untrusted mutations behind a single gate, so the `allowed_paths` strip-merge is the only decision protecting the real repo
...
* **`C-EXEC-07` 🔜: DAL-Escalated Isolation for Pipeline Runs**
  ...
  > - **Needs:** `C-EXEC-06` → the session isolation it escalates into
  ...
  > - **Done when:** 🔴
  > - **Not a one-line flip:** `_derive_allowed_paths` is implement-shaped, so scenario artifacts would be silently dropped by the reconcile gate
...
* **`B-EXEC-01` ✅: Ephemeral Podman Sub-Containers** (Legacy: 3.45)
  ...
  > - **Needs:** `D-VAL-01` → the QA runner it routes through
  ...
  > - **Default:** unsandboxed host execution until explicitly enabled
...
* **`B-EXEC-02` 🔜: Tiered Access Rights** (Legacy: 4.4)
  ...
  > - **Trigger:** 🔴
  > - **Needs:** `E-SENS-03` → the grant model to tier
  > - **Reads:** 🔴
  > - **Produces:** 🔴
  > - **Enables:** 🔴
  > - **Done when:** 🔴
...
* **`B-EXEC-03` 🔜: Blast Radius Enforcement** (Legacy: 4.8)
  ...
  > - **Needs:** `B-SENS-02` → the graph · `TECH-068` → real edges · `B-SENS-08` → honest on framework code
...
* **`B-EXEC-04` 🔜: Kernel-Enforced Resource Bounds (cgroups v2)**
  ...
  > - **Needs:** `B-EXEC-01` → the container boundary · Linux cgroups v2 delegation
  > - **Reads:** —
...
* **`A-EXEC-01` 🔜: Functional Sandboxing (Black Box Ledgers)** (Legacy: 3.46)
  ...
  > - **Enables:** 🔴
  > - **Done when:** 🔴
  > - **Trade:** state determinism over execution speed. Continuous chat context is disabled entirely
...
* **`A-EXEC-02` 🔜: Fuzzing Harnesses** (Legacy: 4.13)
  ...
  > - **Trigger:** 🔴
  > - **Needs:** `D-INTL-08` → the polyglot implement loop · a real native-code target
  > - **Reads:** the generated AST, for C++ / Rust targets
  > - **Produces:** 🟡 `libFuzzer` loops
  > - **Enables:** 🔴
  > - **Done when:** 🔴
  > - **Gate:** the trading system as planned is not a native-code target
...
* **`A-EXEC-04` 🔜: Advanced Row-Level Task Locking**
  ...
  > - **Needs:** `B-INTL-09` → its heartbeat and OCC foundation
  > - **Reads:** 🟡 task rows
