...
| FR-5 | Status mapping | `BashActionHandler` | Maps exit code 0 → `StepStatus.COMPLETED`, any nonzero → `StepStatus.FAILED` | Existing `GateDefinition`/`RouterRule` evaluation works on bash steps with zero engine changes |
