# Implementation Plan: Native CLI Action Nodes [SF-03: Scaffold, Boundary Config, and Docs]

- **Feature ID**: C-EXEC-02
- **Sub-Feature**: SF-03 — Scaffold, Boundary Config, and Docs
...
**Outputs**: `.specweaver/scripts/` exists on every newly-scaffolded project; `tach.toml`/`context.yaml` are ready for SF-02 to pass `tach check`; docs are accurate.
**Depends on**: none.
...
- `docs/dev_guides/pipeline_engine_guide.md`'s `action: bash` pipeline-syntax section (Guide-1) — deferred to SF-02's pre-commit, since the pipeline capability doesn't exist until SF-02 lands.
- Actually wiring `core/flow` code to import `sandbox.execution.core.atom.BashActionAtom` — that's SF-02's job; SF-03 only makes the import tach-legal.

## Phase 5: Final Consistency Check

**5.0 Pre-check**: The single FR assigned to SF-03 (FR-10) is covered. No NFRs or ADs are assigned to this SF.
