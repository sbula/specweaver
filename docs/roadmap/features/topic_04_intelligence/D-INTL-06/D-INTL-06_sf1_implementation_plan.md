# Implementation Plan: Context Hydration & Handover Engine [SF-1: Memory Hydrator & DTO]
- **Feature ID**: D-INTL-06
- **Sub-Feature**: SF-1 — Memory Hydrator & HydrationResult DTO
...
SF-1 implements the read-side context hydration layer for the Agent Memory Bank. It introduces a shared query service (CQRS read-side), a hydrator service, and a DTO for structured prompt injection. All code lives in `workspace/memory/`.
...
**FRs NOT covered (deferred)**: FR-6 (prompt factory → SF-2), FR-7 (PromptContext → SF-2), FR-8 (handover save → SF-3), FR-9 (handover bootstrap → SF-2/SF-3).
...
`workflows.review.interfaces` and `workflows.implementation.interfaces` already consume `workspace.project` — establishing the `workflows → workspace` precedent. Import is legal. SF-1 registers `workspace.memory` as the producer; SF-2 adds `depends_on` as the consumer.
...
| 3 | tach.toml registration | HIGH | **Register now** in SF-1 — import verified legal in code |
...
| 8 | tach scope | MEDIUM | **Yes** — producer declares API in SF-1 |
...
>
> SF-2 adds Layer 6: **System instruction framing** around the `<context label="agent_memory">` block.
...
> [!NOTE]
> **Why now (not SF-2)**: SF-1 defines the public API. The producer declares what it exposes. SF-2 (the consumer) will add `depends_on = ["src.specweaver.workspace.memory"]` to `workflows.commons`.
...
| 3 | `D-INTL-06_design.md` Progress Tracker | Mark SF-1 `Impl Plan ✅` |
...
1. **Prompt factory (FR-6, FR-7)** → SF-2
2. **Handover save/bootstrap (FR-8, FR-9)** → SF-3
...
6. **System instruction framing around memory block** → SF-2 (PromptFactory)
7. **Write-side injection validation** → SF-3 (FR-8 callback, validate summary content BEFORE storing)
