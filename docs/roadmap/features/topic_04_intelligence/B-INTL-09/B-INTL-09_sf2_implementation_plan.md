...
SF-2 implements the foundational `MemoryRepository` class providing core CRUD operations and formal State Transition Matrix enforcement for the Agent Memory Bank. It covers:
...
**FRs explicitly NOT covered (deferred to SF-3 / SF-4)**:
- OCC `acquire_task` with backoff → SF-3
- `WITH RECURSIVE` cycle checks → SF-3
- Pydantic context validation (8KB) → SF-3
- Zombie recovery, circuit breaker, upstream propagation → SF-4

**Inputs**: SQLAlchemy models from SF-1 (`Task`, `Epic`, `TaskDependency`, `StateTransition`, `Defect`, `ALLOWED_TRANSITIONS`, all enums).
...
3. **Return Format**: `WorkspaceRepository` returns `dict[str, object]` from `get_*` and `list_*` methods rather than ORM model instances. `FlowRepository` also uses this pattern. **SF-2 MUST follow this pattern** to maintain consistency.
...
5. **PRAGMA Integration**: SF-1 created `register_fk_pragma_listener()` in `database.py`. The repository itself does NOT need to call this — it is the session creator's responsibility (either `session_scope()` or test fixtures).
...
1. **OCC with `version_id_col`**: SQLAlchemy 2.0 natively supports OCC via `__mapper_args__ = {"version_id_col": version}`. However, this auto-increments `version` on **every flush**, not just on specific state transitions. Since SF-2's scope is basic CRUD + state machine (without OCC acquisition), we will NOT use `version_id_col` mapper args in SF-2. We will manually manage the `version` column. SF-3 will implement the transactional OCC `acquire_task` with explicit version checking and `StaleDataError`-style retry logic.
...
|---|---------|----------|----------|
| 1 | File placement: `repository.py` vs `store.py` | HIGH | **Separate `repository.py`** — MemoryRepository is too complex (13+ methods, grows in SF-3/SF-4) to coexist with schema. |
...
| RT-2 | Defect invariant check is non-atomic (theoretical race window) | Documented as known gap; closed by SF-3 OCC |
...
| `update_handover_context` | `(task_id: uuid.UUID, context: str \| None) → dict[str, object]` | Basic context update (no Pydantic validation — that's SF-3). Validates task exists. |
| `add_task_dependency` | `(parent_id: uuid.UUID, child_id: uuid.UUID) → None` | Add DAG link (RT-15). Validates both tasks exist. Raises ValueError on self-dependency or duplicate link (RT-20). Cycle checks deferred to SF-3. |
...
> [!NOTE]
> **RT-2: Defect invariant race condition**: The defect check (step 4) and status update (step 5) are non-atomic within the same `flush()`. This is a theoretical gap that is practically mitigated by SQLite's WAL write serialization and `NullPool` connection isolation. SF-3's transactional OCC will close this gap for true multi-process concurrent access.
...
> [!NOTE]
> **`version` Column Handling in SF-2**: SF-2 does NOT increment `task.version` during state transitions. The `version` column is reserved for Optimistic Concurrency Control in SF-3's `acquire_task`. SF-2 sets `version=1` on creation and leaves it unchanged. SF-3 will use it to prevent dual-acquisition race conditions.

> [!NOTE]
> **`attempt_count` Column Handling in SF-2**: SF-2 does NOT modify `attempt_count`. It is set to `0` on creation and managed exclusively by SF-4's zombie recovery and circuit breaker logic.
...
**Test fixture strategy**: Reuse the same `engine`/`session`/`base_project` fixture pattern from `test_memory_store.py` (SF-1). The test file will use `@pytest.mark.asyncio` class-based grouping.
...
1. **OCC `acquire_task` with version column** → Deferred to SF-3. SF-2 does not increment `version`.
2. **Pydantic `HandoverContext` validation** → Deferred to SF-3. SF-2 does basic string storage only.
3. **`WITH RECURSIVE` cycle detection on dependency insert** → Deferred to SF-3.
4. **Zombie recovery + circuit breaker + upstream propagation** → Deferred to SF-4.
