# Implementation Plan: Agent Memory Bank [SF-3: DAG & Context Validation]
- **Feature ID**: B-INTL-09
- **Sub-Feature**: SF-3 — DAG & Context Validation
...
- **Implementation Plan**: docs/roadmap/features/topic_04_intelligence/B-INTL-09/B-INTL-09_sf3_implementation_plan.md
- **Status**: APPROVED
...
SF-3 extends the `MemoryRepository` with three capabilities not present in the SF-2 CRUD foundation:
...
**FRs explicitly NOT covered (deferred to SF-4)**:
- Zombie Recovery heartbeat scanning → SF-4
- 3-Strike Circuit Breaker → SF-4
- Upstream `BLOCKED` → `UPSTREAM_BLOCKED` DAG propagation → SF-4

**Inputs**: The `MemoryRepository` core CRUD from SF-2 (`repository.py`), the SQLAlchemy models from SF-1 (`store.py`).
...
- `tests/unit/workspace/test_memory_repository.py` (modified — add SF-3 tests)
- `tests/integration/workspace/test_memory_integration.py` (modified — add SF-3 integration scenarios)
...
1. **Existing `add_task_dependency`**: SF-2 implemented a basic version at `repository.py:269-284` that checks for self-dependency and duplicate edges but does NOT perform `WITH RECURSIVE` cycle detection. SF-3 must **replace** this method with `insert_dependency` that includes cycle checks, OR rename and extend the existing one.

2. **Existing `update_handover_context`**: SF-2 implemented a raw string setter at `repository.py:258-267`. It accepts `str | None` and stores directly. SF-3 must **upgrade** this to validate through a Pydantic model and enforce the 8KB limit at the application boundary. The DB already has a `CheckConstraint("length(handover_context) <= 8192")` as a safety net (SF-1).
...
2. **OCC Pattern**: SQLAlchemy 2.0's native `version_id_col` mapper arg auto-increments version on every flush, which is too aggressive. SF-3 will manually check `version` in a WHERE clause during `acquire_task` and increment only on successful acquisition.
...
**Change**: Add 2 new domain exceptions for SF-3.
...
> [!CAUTION]
> **`remove_task_dependency`** (lines 286-299) is kept as-is from SF-2. It does not need cycle checks since removing an edge can never create a cycle.
...
```python
from sqlalchemy import text, update  # NEW for SF-3
from sqlalchemy.exc import IntegrityError, OperationalError  # NEW for SF-3

from specweaver.workspace.memory.errors import (
    CyclicDependencyError,        # NEW for SF-3
    DefectBlocksCompletionError,
    IllegalStateTransitionError,
    StaleTaskVersionError,         # NEW for SF-3
)
from specweaver.workspace.memory.models import HandoverContext  # NEW for SF-3
```
