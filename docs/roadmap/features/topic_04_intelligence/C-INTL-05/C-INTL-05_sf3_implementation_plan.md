# Implementation Plan: Configurable Prompt Render Profiles [SF-3: Caller Migration & Unification]
- **Feature ID**: C-INTL-05
- **Sub-Feature**: SF-3 — Caller Migration & Unification
...
- **Implementation Plan**: docs/roadmap/features/topic_04_intelligence/C-INTL-05/C-INTL-05_sf3_implementation_plan.md
- **Status**: APPROVED

## Overview

SF-3 completes the C-INTL-05 feature by migrating **all** remaining callsites to use explicit `RenderProfile` constants and eliminating the last two "maintenance islands" — `ArbitrateVerdictHandler` (ad-hoc `PromptBuilder()`) and `DecomposeFeatureHandler` / `FeatureDecomposer` (ad-hoc `PromptBuilder()` bypassing `_build_base_prompt()`).

**Primary scope** (from design doc §SF-3):
...
- `_build_base_prompt()` calls `add_project_metadata()`, `add_constitution()`, `add_standards()` — all silently skipped by slot gating (SF-2) since those slots are NOT in ARBITER's `active_slots`.
...
|----------|----------------|
| `C-INTL-05_design.md` Progress Tracker | Mark `Impl Plan ✅` for SF-3 |
| `C-INTL-05_design.md` Session Handoff | Update to point to `/dev` for SF-3 |
