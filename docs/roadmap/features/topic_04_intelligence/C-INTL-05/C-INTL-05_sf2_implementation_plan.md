# Implementation Plan: Configurable Prompt Render Profiles [SF-2: Profile-Driven Rendering & Builder Refactoring]

- **Feature ID**: C-INTL-05
- **Sub-Feature**: SF-2 — Profile-Driven Rendering & Builder Refactoring
...
- **Implementation Plan**: docs/roadmap/features/topic_04_intelligence/C-INTL-05/C-INTL-05_sf2_implementation_plan.md
- **Status**: DRAFT
...
> It will be removed in SF-3 when all callers are migrated to explicit profiles.
...
**Commit message:**
`feat(C-INTL-05/SF-2): profile-driven rendering and builder refactoring`
...
2. **`_ContentBlock.kind` field** (line 54-55): Currently a plain `str`. SF-2 does NOT change this to `PromptSlot` — that would be a breaking change across all existing tests. Instead, the slot gate compares `slot.value` (str) with the block's `kind` (str). The `PromptSlot` is a `StrEnum`, so `slot == block.kind` works directly.
...
6. **`_build_base_prompt()` has 7 callers** (grep confirmed): `draft.py:80`, `generation.py:150`, `generation.py:252`, `generation.py:445`, `review.py:162`, `review.py:273`. All currently use `include_rules=True` (default) except `draft.py:83` which uses `include_rules=False`. SF-2 keeps `include_rules` working but deprecated. SF-3 migrates all callers.
...
- Confirm all existing tests pass (zero regressions from SF-2)
