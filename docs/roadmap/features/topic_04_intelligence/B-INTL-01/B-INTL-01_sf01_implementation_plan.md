...
## 1. Summary
This implementation plan covers SF-01: wiring the `flow` Orchestrator to natively read the target component's `archetype`, dynamically overlaying the validation YAML, and injecting the memory-safe AST payload down into the Validation DMZ without violating the strict `forbids: loom/*` boundary.
