...
| FR-12 | Two layers | Chunker | Emits a **skeleton** chunk and a **body** chunk per symbol, each labelled with its layer. Splitting and merging run **independently per layer**, and `FR-17`'s totality then binds the **body** layer alone | A signature is deliberately in both. "Never the same lines twice" holds **within** a layer, not across, and the skeleton layer is a projection that is deliberately incomplete `[agreed 2026-08-26]` |
