...
## Context & Motivation
During the implementation of SF-03 (Knowledge Graph Builder), we identified that the
`specweaver.workspace.ast.parsers` directory contains both mechanical Tree-Sitter extraction logic
(parsers) and translation layers (adapters) that map ASTs to other domains (like the Universal Graph
Ontology).
...
- **Mitigation:** Execute this refactoring in complete isolation on a fresh branch
  (`chore/tech-02-ast-restructure`) with absolutely no logic changes. Rely on the automated test
  suite (Unit, Integration, E2E) to verify 100% structural parity.
