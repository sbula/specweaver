# Implementation Plan: Domain-Driven Design Unification [SF-03: Consolidate Sandbox]

- **Feature ID**: TECH-001
- **Sub-Feature**: SF-03 — Consolidate Sandbox
...
- **Implementation Plan**: docs/roadmap/features/topic_07_technical_debt/TECH-001/TECH-001_sf03_implementation_plan.md
- **Status**: APPROVED
...
| AD-2 | Hexagonal architecture per domain | SF-02 pattern: `core/` (sealed) + `interfaces/` (facade). Machine-enforceable via `context.yaml`. |
| AD-3 | NFR-6 (`BaseTool` metaclass) deferred | Out of scope — add to tech debt backlog as TECH-002. |
...
| **NFR-6** Meta-Class Registry (BaseTool) | AD-3: Deferred to TECH-002 | Out of FR-6 scope |
...
#### [MODIFY] `docs/roadmap/features/topic_07_technical_debt/TECH-001/TECH-001_design.md`
- `[x]` Update the Progress Tracker: mark SF-03 `Impl Plan ✅`.

### Commit Boundary 3: NFR-6 tech debt issue

#### [NEW] Tech debt issue for NFR-6 (`BaseTool` metaclass registry)
- `[x]` Add TECH-002 to the technical debt backlog for implementing `BaseTool.__init_subclass__` auto-registration.
...
# Excludes: historical docs in docs/roadmap/features/ (except TECH-001/)
```
