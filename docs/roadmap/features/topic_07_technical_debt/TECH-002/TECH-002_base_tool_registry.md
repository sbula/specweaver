...
## 1. Description
During the execution of **TECH-001 (Monolith Purge)**, we migrated `core/loom` into the `sandbox/` domain-driven hexagonal architecture. However, we discovered a deep architectural constraint preventing true isolation of the domains: the `BaseTool` metaclass automatically registers all subclasses globally upon import.
