...
This is not new to C-EXEC-02's `action: bash` steps — it's true of every existing step type (`action: validate`'s `kind`/`coverage`, `action: generate`'s params, etc.). C-EXEC-02 SF-02 deliberately did **not** special-case bash-step params validation at load time (see its implementation plan's Q1) specifically to avoid introducing the first action-specific exception to this uniform-but-late-failing behavior — the real fix, if wanted, should apply to all step types uniformly, which is what this ticket is for.
