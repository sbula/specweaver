...
| # | Finding | Note |
|---|---|---|
| — | **A reload drops every ghost edge.** `load_from_db` filters both endpoints on `is_active = 1` and a ghost is stored with `0`, so a graph read back out of the database says "nothing depends on this" about every dependency outside the parsed set — the confusion `FR-12` exists to remove. `test_load_ignores_ghost_nodes` asserts this and calls it deliberate, and it holds while every build is a full one; `TECH-070`'s incremental rebuild is where it stops holding. `is_active = 0` means two things and only `file_id` separates them: a ghost carries none, a tombstone carries its path. **`T-DIVERGE`** — overturning a tested decision. A test now pins the present behaviour so the day it matters is loud |
| — | **A Rust trait is not a symbol.** `list_symbols` reports `Impl` and its methods and no trait at all, so `Runner` has no node and every `IMPLEMENTS` edge ghosts. The edge carries the right kind and `FR-12` makes the ghost name the trait, so `FR-10` holds — but `FR-9`'s "traversable" does not reach Rust traits. Fixing it means changing `list_symbols`, whose callers reach well past this ticket |
