...
| FR-8 | Delete `env_vars`, `step_records`, `pipeline_name` | Confirmed (research) zero production readers for any of the three; update their 2 real fallback call sites (`gates.py`, `runner_utils.py`) to the literal default value each already always evaluated to in production | 3 fewer fields carried through every `RunContext`, no behavior change (the fallback was always taken) |
...
| NFR-7 | SF-02: Field count | `RunContext`'s own top-level attribute count SHALL decrease from 32 to ≤16 after FR-6/FR-8. |
...
| AD-8 | SF-02: All 6 new sub-models are `frozen=True` | User's explicit choice (2026-08-06, Phase 6 HITL, resolving RED-2.1) — structurally closes the shallow-copy mutation bug class RED-1.3 found (any future direct `context.<group>.<field> = x` raises immediately instead of silently corrupting a shared instance), at the cost of `hydration.py`'s `plan`/`decomposition` writes changing from direct assignment to `context.plan_context = context.plan_context.model_copy(update={...})` (FR-10 updated accordingly — the resulting *value* is unchanged, the *write mechanism* is not). | No |
...
**Current status**: SF-01 fully implemented, tested, and committed. SF-02's design AND implementation plan are both APPROVED (2026-08-06) — `TECH-006_sf02_implementation_plan.md` fully specifies all 5 commit boundaries (`IsolationPolicy` → `PlanContext` → `RunHandle`+`AnalysisContext`+`ModelAccess` → `GraphContext` → dead-field cleanup), each Red/Blue reviewed. No code written yet — `RunContext` still has 32 flat fields today. TECH-006 is not complete until SF-02 lands.
**Next step**: Trigger `specweaver-dev` against `TECH-006_sf02_implementation_plan.md` — TDD across the 5 commit boundaries, each with its own pre-commit gate and HITL commit stop.
