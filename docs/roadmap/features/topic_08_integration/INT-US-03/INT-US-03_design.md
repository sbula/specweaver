...
| AD-5 | **[RESOLVED — approved 2026-07-17]** `sw implement` forces `enforce_isolation=True` for its QA loop (overridable via config), rather than deferring to the global default-off policy. | The contract says QA "MUST execute **exclusively** inside the sandbox," and LLM-generated code is untrusted by definition. `INT-US-09` shipped isolation as global-default-off to preserve legacy behavior; the autonomous implement loop is exactly the case that warrants default-on, degrading cleanly when git is absent. | No (uses existing policy field). |
...
| AD-7 | **[RESOLVED — approved 2026-07-17]** Under isolation, make the generated (uncommitted) `src/<stem>.py` + `tests/test_<stem>.py` reach the `run_tests` worktree by running the **generate** steps under the same policy (`use_worktree=None`) **and** threading the generated paths into `context.allowed_paths` so `strip_merge` commits (not strips) them into HEAD before the QA step's worktree is checked out. SF-03 must prove it end-to-end (possible short spike). | `execute_in_sandbox` is **per-step**: each step gets a fresh worktree checked out from branch HEAD, torn down after, with inter-step data flowing only via the real repo through `strip_merge` (`runner_utils.py:136-196`, `worktree_ops.py:67-135`). A clean HEAD checkout does **not** carry uncommitted working-tree files, and `strip_merge` discards any path not in `allowed_paths`. INT-US-09 only proved a **committed** test runs worktree-bounded — never freshly generated code. | No (works within the existing per-step model) — validated in SF-03. |
...
**Current status**: Design **APPROVED** (2026-07-17). AD-5/6/7 resolved "yes". AD-6 doc fix applied to
`US-03_integration.md`. **SF-01 implementation plan APPROVED**; development in progress.
**Next step**: `/dev` for SF-01 (TDD from the SF-01 plan's Test Plan).
