# Implementation Plan: AST Drift Detection & AI Root-Cause Analysis [SF-01: AST Drift Engine]
- **Feature ID**: 3.14a
- **Sub-Feature**: SF-01 — AST Drift & Coverage Engine
...
---
## Backlog
- Integration with external `flow/` pipeline hooking (Deferred to SF-02).
...
2. **Architecture Scrutiny**:
   - Ensure `specweaver.assurance.validation` does not import `specweaver.core.config.Database` or `LineageMixin`.
