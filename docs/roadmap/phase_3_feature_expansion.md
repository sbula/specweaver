...
| **3.33** | Topology Provider Abstraction | _(new)_ | Toggle between local isolated in-memory Python representation (NetworkX) for sub-second topological analysis (Bicycle mode) and a unified **PostgreSQL (Apache AGE + pgvector)** sidecar (Rocket mode). Prioritizes Breadth-First-Search (BFS) expansion radiating outward from God Nodes, utilizing embeddings strictly as a discoverability fallback to structural graph topology. |
