...
| **3.33** | Topology Provider Abstraction | _(new)_ | Toggle between local SQLite/BM25 persistence augmented by an isolated in-memory Python representation (NetworkX) for sub-second topological analysis (Bicycle mode), and a unified **PostgreSQL (Apache AGE + pgvector)** sidecar (Rocket mode) to map cross-service GraphRAG topologies and vectors in a single transactional backend. Prioritizes Breadth-First-Search (BFS) expansion radiating outward from God Nodes, utilizing embeddings strictly as a discoverability fallback to structural graph topology. |
