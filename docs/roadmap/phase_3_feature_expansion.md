...
| **3.33** | Topology Provider Abstraction (Rocket Mode) | _(new)_ | Swaps out local mechanisms (`SQLite/BM25` queries and the `Feature 3.32` JSON Semantic Hash Caches) for a formalized `ITopologyProvider` interface. Unlocks 'Rocket Mode' by directly streaming Merkle-tree hashes and Cross-Service Topologies into dedicated Sidecar Graph Databases (FalkorDB, Neo4j, VectorDB) for massive multi-node deployments without polluting local filesystems. |
