...
| **3.32b** | Smart scan exclusions (tiered) | _(inspired by PasteMax)_ | 3-tier file exclusion: binary exts, default patterns (.git, __pycache__), per-project overrides + `.specweaverignore`. *(Note: Due to 3.32 SF-4 Pipeline Execution Optimization, future exclusion tools must integrate natively with `TopologyGraph.stale_nodes` rather than building standalone loops.)* |
