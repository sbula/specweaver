...
| **3.29** 🔧 | Archetype-Based Rule Sets | _(new)_ | Auto-provisioned rules for specific architectural profiles (`kotlin-service`, `rust-worker`) to enforce framework-specific standards inherently. **In progress**: SF-1 (Injection) and SF-3 (Markdown/S12 + Framework plugins) Boundary 3 completed. |
