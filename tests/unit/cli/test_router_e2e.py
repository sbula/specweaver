...
from specweaver.cli.pipelines import _execute_run, resume
...
def test_cli_pipelines_injects_model_router_in_run(
    ...
    with (
        patch("specweaver.cli.pipelines._core.get_db", return_value=mock_active_db),
        patch("specweaver.cli.pipelines.resolve_project_path", return_value=project_dir),
        patch("specweaver.flow.runner.PipelineRunner") as mock_runner_cls,
    ):
...
def test_cli_pipelines_injects_model_router_in_resume(
    ...
    with (
        patch("specweaver.cli.pipelines._core.get_db", return_value=mock_active_db),
        patch("specweaver.cli.pipelines.resolve_project_path", return_value=project_dir),
        patch("specweaver.cli.pipelines._get_state_store", return_value=mock_state_db),
        patch("specweaver.flow.runner.PipelineRunner") as mock_runner_cls,
    ):
        mock_runner = mock_runner_cls.return_value
...
def test_empty_pipeline_spec_edgecase(mock_active_db: MagicMock, tmp_path: Path) -> None:
    ...
    with patch("specweaver.cli.pipelines._core.get_db", return_value=mock_active_db):
