...
class TestHITLProviderAsk:
    ...
    async def test_ask_returns_user_input(self) -> None:
        provider = HITLProvider()
        with patch("specweaver.interfaces.cli.hitl_provider.Prompt.ask", return_value="my answer"):
    ...
    async def test_ask_strips_whitespace(self) -> None:
        provider = HITLProvider()
        with patch("specweaver.interfaces.cli.hitl_provider.Prompt.ask", return_value="  padded  "):
