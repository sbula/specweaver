...
class TestLineageMixin:
    ...
    def test_log_artifact_event_with_parent(self, db):
        db.log_artifact_event("uuid-2", "uuid-1", "run-1", "CREATED")
        history = db.get_artifact_history("uuid-2")
        assert len(history) == 1
        assert history[0]["parent_id"] == "uuid-1"

    def test_multiple_events_same_artifact(self, db):
        db.log_artifact_event("uuid-3", None, "run-1", "CREATED")
        db.log_artifact_event("uuid-3", None, "run-2", "MODIFIED")
    ...
    def test_get_children(self, db):
        db.log_artifact_event("parent-a", None, "run-1", "CREATED")
        db.log_artifact_event("child-1", "parent-a", "run-2", "CREATED")
        db.log_artifact_event("child-2", "parent-a", "run-2", "CREATED")
    ...
    def test_log_artifact_event_null_constraint(self, db):
        with pytest.raises(ValueError, match="cannot be empty"):
            db.log_artifact_event("uuid-1", None, "run-1", None)  # type: ignore
    ...
    def test_log_artifact_event_empty_string_validation(self, db):
        with pytest.raises(ValueError, match="artifact_id cannot be empty"):
            db.log_artifact_event("   ", None, "run-1", "CREATED")

        with pytest.raises(ValueError, match="run_id cannot be empty"):
            db.log_artifact_event("uuid-1", None, "", "CREATED")

        with pytest.raises(ValueError, match="event_type cannot be empty"):
            db.log_artifact_event("uuid-1", None, "run-1", "  ")
