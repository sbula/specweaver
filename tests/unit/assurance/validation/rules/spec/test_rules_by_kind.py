...
from specweaver.assurance.validation.spec_kind import SpecKind
...
class TestS03KindAware:
    ...
    def test_feature_flags_file_paths(self) -> None:
        ...
        from specweaver.assurance.validation.rules.spec.s03_stranger import StrangerTestRule
...
Imports from `specweaver.assurance.validation.runner` to process specs.
"""
        rule = StrangerTestRule(mode="abstraction_leak")
        result = rule.check(spec)
        assert result.status in (Status.WARN, Status.FAIL)
...
Imports from `specweaver.assurance.validation.runner` for processing.
"""
        rule = StrangerTestRule(mode="abstraction_leak")
        result = rule.check(spec)
        assert result.status in (Status.WARN, Status.FAIL)
