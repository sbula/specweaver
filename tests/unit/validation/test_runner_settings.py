...
"""Tests for apply_settings_to_pipeline — threshold/enable overrides via DB settings.
...
All tests use the production pipeline executor path:
    load_pipeline_yaml → apply_settings_to_pipeline → execute_validation_pipeline
...
class TestPipelineSettingsEdgeCases:
    ...
    def test_none_settings_returns_same_pipeline(self, spec_pipeline) -> None:
        """apply_settings_to_pipeline(pipeline, None) returns the same pipeline."""
    ...
    def test_execute_after_settings_produces_results(self, spec_pipeline) -> None:
        """After applying settings, execution still returns results for all remaining steps."""
        settings = ValidationSettings(overrides={
            "S08": RuleOverride(rule_id="S08", enabled=False),
        })
        pipeline = apply_settings_to_pipeline(spec_pipeline, settings)
