...
class TestMemoryRepositoryEpics:
    ...
    async def test_create_epic_empty_title(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_close_epic_already_closed(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_close_epic_updates_timestamp(self, session: AsyncSession, base_project: Project) -> None:
...
class TestMemoryRepositoryTasks:
    ...
    async def test_create_task_sets_timestamps(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_create_task_empty_title(self, session: AsyncSession, base_project: Project) -> None:
        """RT-3: Raises ValueError for empty/whitespace title."""
        repo = MemoryRepository(session)
        with pytest.raises(ValueError, match="title cannot be empty or whitespace-only"):
            await repo.create_task(project_name=base_project.name, title="   ")

    async def test_create_task_with_epic(self, session: AsyncSession, base_project: Project) -> None:
        """Happy Path: Task linked to existing epic."""
        repo = MemoryRepository(session)
        epic = await repo.create_epic(project_name=base_project.name, title="Epic")
        task = await repo.create_task(project_name=base_project.name, title="Task", epic_id=uuid.UUID(epic["id"]))
        assert task["epic_id"] == epic["id"]

    async def test_create_task_invalid_epic(self, session: AsyncSession, base_project: Project) -> None:
        """Boundary: Raises ValueError for nonexistent epic_id."""
        repo = MemoryRepository(session)
        with pytest.raises(ValueError, match="Epic not found"):
            await repo.create_task(project_name=base_project.name, title="Task", epic_id=uuid.uuid4())

    async def test_create_task_epic_project_mismatch(self, session: AsyncSession, base_project: Project) -> None:
        ...
        with pytest.raises(ValueError, match="Epic belongs to a different project"):
            await repo.create_task(project_name=base_project.name, title="Task", epic_id=uuid.UUID(epic["id"]))
    ...
    async def test_list_tasks_filtered_by_status(self, session: AsyncSession, base_project: Project) -> None:
        ...
        pending_tasks = await repo.list_tasks(project_name=base_project.name, status=TaskStatus.PENDING)
    ...
    async def test_list_tasks_empty_project(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_update_task_empty_title(self, session: AsyncSession, base_project: Project) -> None:
...
class TestMemoryRepositoryDefects:
    async def test_create_defect_happy_path(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_create_defect_empty_title(self, session: AsyncSession, base_project: Project) -> None:
        """RT-3: Raises ValueError for empty/whitespace title."""
        repo = MemoryRepository(session)
        task = await repo.create_task(project_name=base_project.name, title="T1")
        with pytest.raises(ValueError, match="title cannot be empty or whitespace-only"):
    ...
    async def test_resolve_defect_already_resolved(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_list_defects_filtered(self, session: AsyncSession, base_project: Project) -> None:
        ...
        open_defects = await repo.list_defects(task_id=uuid.UUID(task["id"]), status=DefectStatus.OPEN)
        assert len(open_defects) == 1
        assert open_defects[0]["title"] == "Bug 2"

        resolved_defects = await repo.list_defects(task_id=uuid.UUID(task["id"]), status=DefectStatus.RESOLVED)
        assert len(resolved_defects) == 1
        assert resolved_defects[0]["title"] == "Bug 1"

    async def test_structured_logging_on_defect_create(self, session: AsyncSession, base_project: Project, caplog) -> None:
        ...
        assert any("Defect created" in record.message and str(defect["id"]) in record.message for record in caplog.records)

    async def test_structured_logging_on_defect_resolve(self, session: AsyncSession, base_project: Project, caplog) -> None:
        ...
        assert any("Defect resolved" in record.message and str(defect["id"]) in record.message for record in caplog.records)


@pytest.mark.asyncio
class TestMemoryRepositoryDependencies:
    async def test_update_handover_context(self, session: AsyncSession, base_project: Project) -> None:
        """Happy Path: Sets/clears handover_context on task."""
        repo = MemoryRepository(session)
        task = await repo.create_task(project_name=base_project.name, title="T1")

        updated = await repo.update_handover_context(task_id=uuid.UUID(task["id"]), context='{"state": "test"}')
        assert updated["handover_context"] == '{"state": "test"}'
    ...
    async def test_update_handover_context_task_not_found(self, session: AsyncSession) -> None:
        ...
        repo = MemoryRepository(session)
        with pytest.raises(ValueError, match="Task not found"):
            await repo.update_handover_context(task_id=uuid.uuid4(), context='{}')

    async def test_update_handover_context_updates_timestamp(self, session: AsyncSession, base_project: Project) -> None:
        """RT-4: Verifies updated_at is bumped."""
        repo = MemoryRepository(session)
        task = await repo.create_task(project_name=base_project.name, title="T1")

        updated = await repo.update_handover_context(task_id=uuid.UUID(task["id"]), context='{}')
        assert updated["updated_at"] > task["updated_at"]

    async def test_add_task_dependency_happy_path(self, session: AsyncSession, base_project: Project) -> None:
        """Happy Path: Successfully creates TaskDependency link (RT-15)."""
        repo = MemoryRepository(session)
        t1 = await repo.create_task(project_name=base_project.name, title="T1")
        t2 = await repo.create_task(project_name=base_project.name, title="T2")

        await repo.add_task_dependency(parent_id=uuid.UUID(t1["id"]), child_id=uuid.UUID(t2["id"]))

        # Verify directly in DB since list_dependencies is not a requirement
    ...
    async def test_add_task_dependency_self_loop(self, session: AsyncSession, base_project: Project) -> None:
        """Boundary: Raises ValueError when parent_id == child_id (RT-15)."""
        repo = MemoryRepository(session)
        t1 = await repo.create_task(project_name=base_project.name, title="T1")

        with pytest.raises(ValueError, match="Cannot add self-dependency"):
            await repo.add_task_dependency(parent_id=uuid.UUID(t1["id"]), child_id=uuid.UUID(t1["id"]))

    async def test_add_task_dependency_duplicate(self, session: AsyncSession, base_project: Project) -> None:
        """RT-20: Raises ValueError when link already exists."""
        repo = MemoryRepository(session)
        t1 = await repo.create_task(project_name=base_project.name, title="T1")
        t2 = await repo.create_task(project_name=base_project.name, title="T2")
    ...
    async def test_remove_task_dependency(self, session: AsyncSession, base_project: Project) -> None:
        """Happy Path: Successfully removes TaskDependency link (RT-15)."""
        repo = MemoryRepository(session)
        t1 = await repo.create_task(project_name=base_project.name, title="T1")
        t2 = await repo.create_task(project_name=base_project.name, title="T2")

        await repo.add_task_dependency(parent_id=uuid.UUID(t1["id"]), child_id=uuid.UUID(t2["id"]))
        await repo.remove_task_dependency(parent_id=uuid.UUID(t1["id"]), child_id=uuid.UUID(t2["id"]))
    ...
    async def test_remove_task_dependency_not_found(self, session: AsyncSession, base_project: Project) -> None:
...
class TestMemoryRepositoryStateMachine:
    async def test_transition_state_happy_path(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_transition_state_illegal(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_transition_state_updates_timestamp(self, session: AsyncSession, base_project: Project) -> None:
        """RT-4: Verifies updated_at is bumped."""
        from specweaver.workspace.memory.store import TaskStatus
        repo = MemoryRepository(session)
        task = await repo.create_task(project_name=base_project.name, title="T1")
    ...
    async def test_transition_state_creates_audit_trail(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_transition_state_task_not_found(self, session: AsyncSession) -> None:
        ...
        repo = MemoryRepository(session)
        with pytest.raises(ValueError, match="Task not found"):
            await repo.transition_state(task_id=uuid.uuid4(), to_status=TaskStatus.IN_PROGRESS, reason="")

    async def test_transition_state_bump_attempt_count(self, session: AsyncSession, base_project: Project) -> None:
        """RT-22: IN_PROGRESS -> BLOCKED bumps attempt_count."""
        from specweaver.workspace.memory.store import TaskStatus
        repo = MemoryRepository(session)
        task = await repo.create_task(project_name=base_project.name, title="T1")
        await repo.transition_state(task_id=uuid.UUID(task["id"]), to_status=TaskStatus.IN_PROGRESS, reason="start")
    ...
    async def test_transition_state_clear_locked_at(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_transition_state_to_done_with_open_defects(self, session: AsyncSession, base_project: Project) -> None:
        ...
        with pytest.raises(DefectBlocksCompletionError):
            await repo.transition_state(task_id=uuid.UUID(task["id"]), to_status=TaskStatus.DONE, reason="done")

    async def test_transition_state_to_done_without_defects(self, session: AsyncSession, base_project: Project) -> None:
        ...
        updated = await repo.transition_state(task_id=uuid.UUID(task["id"]), to_status=TaskStatus.DONE, reason="done")
    ...
    async def test_get_task_transitions(self, session: AsyncSession, base_project: Project) -> None:
        ...
        repo = MemoryRepository(session)
        task = await repo.create_task(project_name=base_project.name, title="T1")
        await repo.transition_state(task_id=uuid.UUID(task["id"]), to_status=TaskStatus.IN_PROGRESS, reason=TransitionReason.ACQUIRED)
        await repo.transition_state(task_id=uuid.UUID(task["id"]), to_status=TaskStatus.BLOCKED, reason=TransitionReason.MANUAL_UNBLOCK)
    ...
    async def test_get_task_transitions_empty(self, session: AsyncSession, base_project: Project) -> None:
    ...
    async def test_structured_logging_on_transition(self, session: AsyncSession, base_project: Project, caplog) -> None:
        ...
        assert any("Task transition" in record.message and "PENDING → IN_PROGRESS" in record.message for record in caplog.records)

    async def test_structured_logging_on_illegal_transition(self, session: AsyncSession, base_project: Project, caplog) -> None:
        ...
        with pytest.raises(IllegalStateTransitionError):
            await repo.transition_state(task_id=uuid.UUID(task["id"]), to_status=TaskStatus.DONE, reason="nope")

        assert any("Illegal state transition" in record.message for record in caplog.records)

    async def test_transition_archived_clears_context(self, session: AsyncSession, base_project: Project) -> None:
        """FR-7: Transition to ARCHIVED sets handover_context = None."""
        from specweaver.workspace.memory.store import TaskStatus, TransitionReason
        repo = MemoryRepository(session)
        task = await repo.create_task(project_name=base_project.name, title="T1")
        await repo.update_handover_context(task_id=uuid.UUID(task["id"]), context="some context")
        await repo.transition_state(task_id=uuid.UUID(task["id"]), to_status=TaskStatus.IN_PROGRESS, reason=TransitionReason.ACQUIRED)
        await repo.transition_state(task_id=uuid.UUID(task["id"]), to_status=TaskStatus.DONE, reason=TransitionReason.COMPLETED)

        updated = await repo.transition_state(task_id=uuid.UUID(task["id"]), to_status=TaskStatus.ARCHIVED, reason=TransitionReason.ARCHIVED)
        assert updated["handover_context"] is None

    async def test_structured_logging_on_blocked(self, session: AsyncSession, base_project: Project, caplog) -> None:
        ...
        await repo.transition_state(task_id=uuid.UUID(task["id"]), to_status=TaskStatus.BLOCKED, reason=TransitionReason.MANUAL_UNBLOCK)
        assert any("Task transition" in record.message and "PENDING → BLOCKED" in record.message for record in caplog.records)

    async def test_transition_all_valid_paths(self, session: AsyncSession, base_project: Project) -> None:
        ...
        for from_status, allowed in ALLOWED_TRANSITIONS.items():
            for to_status in allowed:
                task_dict = await repo.create_task(project_name=base_project.name, title=f"T_val_{from_status.name}_{to_status.name}")
                ...
                updated = await repo.transition_state(task_id=task_id, to_status=to_status, reason=TransitionReason.MANUAL_UNBLOCK)
                assert updated["status"] == to_status.value

    async def test_transition_all_invalid_paths(self, session: AsyncSession, base_project: Project) -> None:
        ...
        for from_status in TaskStatus:
            ...
            for to_status in TaskStatus:
                ...
                task_dict = await repo.create_task(project_name=base_project.name, title=f"T_inv_{from_status.name}_{to_status.name}")
                task_id = uuid.UUID(task_dict["id"])
                ...
                with pytest.raises(IllegalStateTransitionError):
                    await repo.transition_state(task_id=task_id, to_status=to_status, reason=TransitionReason.MANUAL_UNBLOCK)
