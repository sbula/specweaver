...
async def test_workspace_store_happy_path_crud(engine):
    async with session_scope(engine) as session:
        ...
        project = Project(
            name="test-project", root_path="/tmp/test", created_at=now, last_used_at=now
        )
        session.add(project)
...
async def test_workspace_store_boundary_composite_pk(engine):
    async with session_scope(engine) as session:
        now = datetime(2026, 5, 2, 10, 0, 0, tzinfo=UTC)
        project = Project(name="proj1", root_path="/tmp/1", created_at=now, last_used_at=now)
        session.add(project)

        # Valid: Differing scopes
        std1 = ProjectStandard(
            project_name="proj1",
            scope="global",
            language="python",
            category="lint",
