...
def test_hooks_e2e_native_git_commit(tmp_path: Path):
    ...
    install_cmd = [
        sys.executable,
        "-m",
        "specweaver.interfaces.cli.main",
        "hooks",
        "install",
        "--project",
        str(project_dir),
    ...
    res = subprocess.run(
        install_cmd, cwd=project_dir, capture_output=True, text=True, encoding="utf-8"
    )
    ...
    assert "AST signatures match specification." in res_commit.stderr
