...
async def test_cli_execution_suppresses_debug_and_writes_json(tmp_path: Path) -> None:
    ...
    with patch('specweaver.telemetry_logger._get_logs_dir', return_value=tmp_path):
        with patch('specweaver.workspace.project.interfaces.cli._run_workspace_op', side_effect=_mock_op):
            with patch('specweaver.interfaces.cli.main._run_workspace_op', side_effect=_mock_op):
                with patch('specweaver.core.config.cli_db_utils.get_db'):
                    with patch('specweaver.interfaces.cli._core.get_db'):
                        teardown_logging()
                        ...
                        try:
                            ...
                            for line in content.strip().split('\n'):
                                try:
                                    ...
                                    if record.get('levelname') == 'DEBUG' and 'Executing projects command' in record.get('message', ''):
                                        found = True
                                        break
