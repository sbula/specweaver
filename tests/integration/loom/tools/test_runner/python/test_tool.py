...
def test_python_tool_integration(python_project: Path) -> None:
    ...
    res_tests = tool.run_tests(target="tests/unit/loom/commons/test_runner/python/")
    assert res_tests.status == "success", res_tests.message

    res_lint = tool.run_linter(target="tests/unit/loom/commons/test_runner/python/")
    assert res_lint.status in ["success", "error"]

    res_complex = tool.run_complexity(target="tests/unit/loom/commons/test_runner/python/", max_complexity=10)
