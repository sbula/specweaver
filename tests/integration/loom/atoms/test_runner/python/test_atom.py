...
def test_python_atom_integration(python_project: Path) -> None:
    ...
    atom = TestRunnerAtom(cwd=python_project, language="python")
    ...
    res_tests = atom.run({"intent": "run_tests", "target": "tests/unit/loom/commons/test_runner/python/"})
    assert res_tests.status == AtomStatus.SUCCESS, res_tests.message

    # 3. Linter
    res_lint = atom.run({"intent": "run_linter", "target": "tests/unit/loom/commons/test_runner/python/"})
    assert res_lint.status in [AtomStatus.SUCCESS, AtomStatus.FAILED]

    # 4. Complexity
    res_complex = atom.run({"intent": "run_complexity", "target": "tests/unit/loom/commons/test_runner/python/", "max_complexity": 10})
