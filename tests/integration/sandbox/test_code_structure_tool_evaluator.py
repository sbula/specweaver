...
def test_code_structure_atom_read_unrolled_symbol(tmp_path, mock_schemas):
    ...
    atom = CodeStructureAtom(
        file_executor=executor, evaluator_schemas=mock_schemas, active_archetype="fastapi"
    )

    context = {"intent": "read_unrolled_symbol", "path": "test.py", "symbol_name": "User"}

    result = atom.run(context)

    assert result.status.value == "SUCCESS", result.message
    assert (
        "# [Framework Eval] Python standard library dataclass generator."
        in result.exports["symbol"]
    )
...
def test_atom_evaluator_comment_routing_by_extension(tmp_path):
    ...
    atom_java = CodeStructureAtom(
        file_executor=executor, evaluator_schemas=schemas, active_archetype="spring-boot"
    )
    res_j = atom_java.run(
        {"intent": "read_unrolled_symbol", "path": "test.java", "symbol_name": "User"}
    )
    ...
    atom_py = CodeStructureAtom(
        file_executor=executor, evaluator_schemas=schemas, active_archetype="fastapi"
    )
    res_p = atom_py.run(
        {"intent": "read_unrolled_symbol", "path": "test.py", "symbol_name": "User"}
    )
    assert res_p.status.value == "SUCCESS"
    assert (
        "# [Framework Eval] Python standard library dataclass generator." in res_p.exports["symbol"]
    )
...
def test_atom_cyclic_recursion_engine_safety(tmp_path):
    ...
    schemas = {
        "fastapi": {
            "metadata": {"supported_languages": ["python"]},
            "decorators": {"SelfReferencing": ">>{SelfReferencing}<<"},
        }
    }
    fpy = tmp_path / "test.py"
...
def test_tool_blocks_invalid_folder_grant(tmp_path, mock_schemas):
    ...
    tool = CodeStructureTool(
        atom=atom,
        role="implementer",
        grants=[FolderGrant(path="src", mode=AccessMode.FULL, recursive=True)],
    )
    res = tool.read_unrolled_symbol("test.py", "User")
...
def test_tool_dispatcher_intent_hide_with_plugins(tmp_path):
    ...
    assert (
        "read_file_structure" in tool_names
...
""")
    ...
    res = atom.run(
        {"intent": "list_symbols", "path": "Controller.java", "decorator_filter": "RestController"}
    )
