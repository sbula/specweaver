...
async def test_pipeline_runner_sandbox_bouncer(tmp_path: Path):
    ...
    with (
        patch(
            "specweaver.loom.atoms.git.atom.GitAtom.run", autospec=True, side_effect=fake_atom_run
        ),
        patch("specweaver.flow.runner.StepHandlerRegistry.get") as mock_get_handler,
    ):
...
async def test_bouncer_strip_merge_fail_resilience(tmp_path: Path):
    ...
    with (
        ...
        patch("specweaver.flow.runner.StepHandlerRegistry.get") as mock_get,
    ):
        mock_handler = MagicMock()
...
async def test_symlink_cache_folders(tmp_path: Path):
    ...
    with patch("specweaver.loom.atoms.git.atom.GitAtom.run", autospec=True) as mock_atom, patch("os.symlink") as mock_symlink:
