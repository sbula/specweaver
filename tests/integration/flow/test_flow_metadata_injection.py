...
from specweaver.flow._generation import GenerateTestsHandler, PlanSpecHandler
...
@pytest.mark.asyncio
async def test_metadata_flows_to_reviewer_handler(tmp_path: Path, mock_metadata: ProjectMetadata, dummy_step: PipelineStep) -> None:
    mock_llm = AsyncMock()
...
@pytest.mark.asyncio
async def test_metadata_flows_to_generator_handler(tmp_path: Path, mock_metadata: ProjectMetadata, dummy_step: PipelineStep) -> None:
    mock_llm = AsyncMock()
    del mock_llm.generate_with_tools
...
@pytest.mark.asyncio
async def test_metadata_flows_to_planner_handler(tmp_path: Path, mock_metadata: ProjectMetadata, dummy_step: PipelineStep) -> None:
    mock_llm = AsyncMock()
    del mock_llm.generate_with_tools
    plan_data = {"spec_path": "x", "spec_name": "x", "spec_hash": "x", "file_layout": [], "architecture": {"module_layout": "flat", "dependency_direction": "downward", "archetype": "plugin"}, "reasoning": "ok", "confidence": 100}
    mock_llm.generate.return_value = LLMResponse(text=f"<plan>\n{json.dumps(plan_data)}\n</plan>", model="test", cached=False)
