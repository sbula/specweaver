...
async def test_handover_persistence_e2e(tmp_path: Path):
    ...
    with sync_engine.begin() as conn:
        conn.execute(
            Base.metadata.tables["projects"].insert().values(
                name="integration",
                root_path=".",
    ...
    run.step_records.append(StepRecord(
        step_name="int",
        result=StepResult(
...
async def test_handover_persisted_on_failure(tmp_path: Path):
    ...
    with sync_engine.begin() as conn:
        conn.execute(
            Base.metadata.tables["projects"].insert().values(
                name="integration", root_path=".", created_at=datetime.now(UTC), last_used_at=datetime.now(UTC)
            )
        )
    sync_engine.dispose()
    ...
    async with db.async_session_scope() as session:
        task_id = uuid.uuid4()
        task = Task(id=task_id, title="Integration Task", project_name="integration", status=TaskStatus.IN_PROGRESS, created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
        session.add(task)
        await session.commit()

    ctx = RunContext(project_path=Path("."), spec_path=Path("."), db=db, task_id=str(task_id))
    run = PipelineRun(run_id=str(uuid.uuid4()), pipeline_name="integration", project_name="integration", spec_path=".", started_at=datetime.now(UTC).isoformat(), updated_at=datetime.now(UTC).isoformat(), status=RunStatus.FAILED)
    from specweaver.core.flow.engine.state import StepRecord
    run.step_records.append(StepRecord(step_name="int", result=StepResult(status=StepStatus.ERROR, error_message="Integration Failed", started_at=datetime.now(UTC).isoformat(), completed_at=datetime.now(UTC).isoformat())))
...
async def test_handover_noop_when_no_task(tmp_path: Path):
    ...
    run = PipelineRun(run_id=str(uuid.uuid4()), pipeline_name="integration", project_name="integration", spec_path=".", started_at=datetime.now(UTC).isoformat(), updated_at=datetime.now(UTC).isoformat(), status=RunStatus.COMPLETED)
