...
- **Dynamic Tool Gating (Feature 3.30a)** — Intercepts the `context.yaml` active archetype to mathematically remove or inject specific JSON Schema Tool Definitions (`list_symbols`) to the Agent at generation runtime, strictly enforcing framework-specific capabilities.
...
- **Interactive Sandbox Execution (Feature 3.28)** — Dual-pipeline orchestration engine featuring a strict read-only Arbiter component that mediates faults between Coding and Polyglot Scenario Generation pipelines to prevent hallucination ping-pong.
...
| `sw lineage tag <file>` | Inject or update a `# sw-artifact` UUID tag in a source file |
| `sw lineage tree <uuid\|file>` | View the interactive upstream/downstream lineage tree graph |
