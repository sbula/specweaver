...
class MemoryRepository:
    ...
    def _epic_to_dict(epic: Epic) -> dict[str, object]:
        return {
            ...
            "created_at": epic.created_at.isoformat() if isinstance(epic.created_at, datetime) else epic.created_at,
            "updated_at": epic.updated_at.isoformat() if isinstance(epic.updated_at, datetime) else epic.updated_at,
        }
    ...
    async def create_epic(self, project_name: str, title: str, description: str | None = None) -> dict[str, object]:
    ...
    async def list_epics(self, project_name: str) -> list[dict[str, object]]:
        ...
        stmt = select(Epic).where(Epic.project_name == project_name).order_by(Epic.created_at.desc())
        result = await self.session.execute(stmt)
    ...
    async def create_task(self, project_name: str, title: str, description: str | None = None, epic_id: uuid.UUID | None = None) -> dict[str, object]:
        """Create a new Task."""
        _validate_non_empty("title", title)
        await self._ensure_project_exists(project_name)
    ...
    async def list_tasks(self, project_name: str, *, status: TaskStatus | None = None) -> list[dict[str, object]]:
    ...
    async def update_task(self, task_id: uuid.UUID, *, title: str | None = None, description: str | None = None) -> dict[str, object]:
    ...
    def _defect_to_dict(defect: Defect) -> dict[str, object]:
        return {
            ...
            "created_at": defect.created_at.isoformat() if isinstance(defect.created_at, datetime) else defect.created_at,
            "resolved_at": defect.resolved_at.isoformat() if isinstance(defect.resolved_at, datetime) and defect.resolved_at else defect.resolved_at,
        }

    async def create_defect(self, task_id: uuid.UUID, title: str, description: str | None = None) -> dict[str, object]:
    ...
    async def list_defects(self, task_id: uuid.UUID, *, status: DefectStatus | None = None) -> list[dict[str, object]]:
    ...
    async def update_handover_context(self, task_id: uuid.UUID, context: str | None) -> dict[str, object]:
        """Update handover context for a task."""
        task = await self.session.get(Task, task_id)
        if task is None:
            raise ValueError(f"Task not found: {task_id}")

        task.handover_context = context
        task.updated_at = datetime.now(UTC)
        await self.session.flush()
        return self._task_to_dict(task)

    async def add_task_dependency(self, parent_id: uuid.UUID, child_id: uuid.UUID) -> None:
        """Add a task dependency (parent -> child)."""
    ...
    def _transition_to_dict(transition: StateTransition) -> dict[str, object]:
        return {
            ...
            "reason": transition.reason.value if isinstance(transition.reason, TransitionReason) else transition.reason,
            "timestamp": transition.timestamp.isoformat() if isinstance(transition.timestamp, datetime) else transition.timestamp,
        }

    async def transition_state(self, task_id: uuid.UUID, to_status: TaskStatus, reason: TransitionReason | str) -> dict[str, object]:
        ...
        if to_status not in allowed:
            logger.error("Illegal state transition for task %s: %s -> %s", task_id, from_status, to_status)
        ...
        if to_status == TaskStatus.DONE:
            stmt = select(Defect).where(Defect.task_id == task_id, Defect.status == DefectStatus.OPEN)
            res = await self.session.execute(stmt)
        ...
        # 5. Audit Trail
        transition_reason = reason if isinstance(reason, TransitionReason) else TransitionReason.MANUAL_UNBLOCK
        ...
        if to_status in (TaskStatus.BLOCKED, TaskStatus.UPSTREAM_BLOCKED):
            logger.warning("Task transition: task_id=%s, %s → %s, reason=%s", task_id, from_status.value, to_status.value, transition_reason.value)
        else:
            logger.info("Task transition: task_id=%s, %s → %s, reason=%s", task_id, from_status.value, to_status.value, transition_reason.value)
        return self._task_to_dict(task)

    async def get_task_transitions(self, task_id: uuid.UUID) -> list[dict[str, object]]:
        """Fetch audit trail for a task."""
        stmt = select(StateTransition).where(StateTransition.task_id == task_id).order_by(StateTransition.timestamp.asc())
        result = await self.session.execute(stmt)
