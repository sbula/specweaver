...
class TraceabilityRule(Rule):
    ...
    def check(self, spec_text: str, spec_path: Path | None = None) -> RuleResult:
        """Parse spec text for requirements and map them to AST trace comments in test files."""
        # 1. Extract targets greedily: "FR-1", "NFR-2"
        target_ids = set(re.findall(r"\b(?:N)?FR-\d+\b", spec_text))

        if not target_ids:
            return self._pass("No explicit requirement tags (FR-x/NFR-x) found in spec.")

        # 2. Extract found traces dynamically
        mapped_ids = self._find_and_parse_tests(spec_path)
    ...
    def _discover_test_files(self, spec_path: Path | None) -> list[Path]:
        """Crawl upwards to find project root and glob all test files."""
        if spec_path is None:
            return []

        project_root = spec_path.parent
        while project_root != project_root.parent:
            if (project_root / "pyproject.toml").exists() or (project_root / "package.json").exists() or (project_root / ".git").exists():
                break
    ...
    def _find_and_parse_tests(self, spec_path: Path | None) -> set[str]:
        """Find test files and parse ASTs to extract trace tags cleanly from comments."""
        test_files = self._discover_test_files(spec_path)
        if not test_files:
            return set()
